...
async function updateUI() {
  ...
  if (pageState.file) {
    ...
    try {
      ...
      if (pagePositionHint) {
        pagePositionHint.textContent = `Adj meg 0-t az elejére történő beszúráshoz, vagy ${pageCount}-t a végére való beszúráshoz.`;
      }
...
async function addBlankPages() {
  ...
  if (isNaN(position) || position < 0 || position > totalPages) {
    showAlert(
      'Érvénytelen bevitel',
      `Adj meg egy számot 0 és ${totalPages} között.`
    );
    return;
  }
  ...
  try {
    ...
    console.error(e);
    showAlert('Hiba', `Az üres oldal beszúrása nem sikerült.`);
  } finally {
    hideLoader();
  }
}
