...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (state.files.length > 0) {
      ...
      for (let index = 0; index < state.files.length; index++) {
        ...
        try {
          ...
          metaSpan.textContent = `${formatBytes(file.size)} • ${pdfDoc.numPages} oldal`;
        } catch {
          metaSpan.textContent = `${formatBytes(file.size)} • Az oldalszám betöltése nem sikerült`;
        }
      }
  ...
  const displayImages = () => {
    ...
    extractedImages.forEach((img) => {
      ...
      card.className = 'bg-slate-700 rounded-lg overflow-hidden';
      ...
      name.className = 'text-xs text-slate-300 truncate';
      name.textContent = img.name;

      const downloadBtn = document.createElement('button');
      downloadBtn.className = 'text-accent hover:text-accent-light';
  ...
  const extract = async () => {
    try {
      if (state.files.length === 0) {
        showAlert('Nincs fájl', 'Please select at least one PDF file.');
        return;
      }

      const decryptedFiles = await batchDecryptIfNeeded(state.files);
      showLoader('PDF-feldolgozó betöltése...');
