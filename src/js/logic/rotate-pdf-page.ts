...
function createPageWrapper(
  ...
  controls.className =
    'flex items-center justify-center gap-2 p-2 bg-slate-800';

  const rotateLeftBtn = document.createElement('button');
  rotateLeftBtn.className =
    'flex items-center gap-1 px-3 py-1.5 bg-slate-700 hover:bg-slate-600 text-white rounded border border-slate-600 text-xs cursor-pointer';
...
async function applyRotations() {
  ...
  try {
    ...
    console.error(e);
    showAlert('Hiba', 'Could not apply rotations.');
  } finally {
    hideLoader();
  }
}
