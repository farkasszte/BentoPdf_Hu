...
async function updateUI() {
  ...
  if (pageState.file) {
    ...
    try {
      ...
      metaSpan.textContent = `${formatBytes(pageState.file.size)} • ${pageCount} oldal`;
...
function updateAttachmentList() {
  ...
  pageState.attachments.forEach(function (file) {
    ...
    div.className =
      'flex justify-between items-center p-2 bg-slate-800 rounded-md text-white';
...
async function addAttachments() {
  if (!pageState.file || !pageState.pdfDoc) {
    showAlert('Hiba', 'Előbb tölts fel egy PDF-et.');
    return;
  }

  if (pageState.attachments.length === 0) {
    showAlert('Nincs fájl', 'Válassz ki legalább egy csatolandó fájlt.');
    return;
  }
  ...
  try {
    ...
    showAlert(
      'Hiba',
      `Failed to attach files: ${error instanceof Error ? error.message : 'Ismeretlen hiba'}`
    );
  }
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (attachmentInput && attachmentDropZone) {
    ...
    attachmentDropZone.addEventListener('dragover', function (e) {
      e.preventDefault();
      attachmentDropZone.classList.add('bg-slate-700');
    });

    attachmentDropZone.addEventListener('dragleave', function (e) {
      e.preventDefault();
      attachmentDropZone.classList.remove('bg-slate-700');
    });
