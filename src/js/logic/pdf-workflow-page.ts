...
async function initializePage() {
  ...
  document.getElementById('run-btn')?.addEventListener('click', async () => {
    ...
    if (allNodes.length === 0) {
      showAlert('Hiba', 'Add at least one node to run the workflow.');
      return;
    }
    ...
    try {
      ...
      if (statusText) statusText.textContent = 'A munkafolyamat befejeződött';
    } catch (err) {
      if (statusText) statusText.textContent = 'Hiba a végrehajtás során';
      showAlert('Hiba', (err as Error).message);
    } finally {
  ...
  document.getElementById('clear-btn')?.addEventListener('click', async () => {
    ...
    updateNodeCount();
    const statusText = document.getElementById('status-text');
    if (statusText) statusText.textContent = 'Kész';
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Törlés' || e.key === 'Backspace') {
...
function showSaveTemplateModal(
  ...
  const doSave = () => {
    ...
    if (!name) {
      errorEl.textContent = 'Adj meg egy nevet.';
      errorEl.classList.remove('hidden');
      return;
    }
    if (templateNameExists(name)) {
      errorEl.textContent = 'Már létezik ilyen nevű sablon.';
      errorEl.classList.remove('hidden');
      return;
    }
...
function showLoadTemplateModal(
  ...
  if (names.length === 0) {
    ...
    for (const name of names) {
      ...
      row.className =
        'group flex items-center gap-2 bg-slate-950/60 hover:bg-slate-700/50 rounded-lg px-3 py-2.5 border border-slate-700/50 transition-colors cursor-pointer';

      const icon = document.createElement('i');
      icon.className = 'ph ph-file-text text-base text-slate-500 flex-shrink-0';
      row.appendChild(icon);

      const label = document.createElement('span');
      label.className = 'text-slate-200 text-sm truncate flex-1';
      label.textContent = name;
      row.appendChild(label);

      const loadBtn = document.createElement('button');
      loadBtn.className =
        'bg-accent hover:bg-accent text-white text-xs font-medium px-3 py-1.5 rounded-md transition-colors flex-shrink-0';
      ...
      loadBtn.addEventListener('click', async () => {
        ...
        if (loaded) {
          ...
          showAlert('Hiba', 'Failed to load template.');
        }
      });
      ...
      delBtn.className =
        'text-slate-600 hover:text-red-400 transition-colors flex-shrink-0';
...
function buildToolbox() {
  ...
  const categoryOrder: Array<{
    ...
    { key: 'Edit & Annotate', color: 'text-accent-light' },
  ...
  for (const cat of categoryOrder) {
    ...
    for (const entry of entries) {
      ...
      item.className =
        'toolbox-node-item w-full text-left px-2 py-1.5 rounded-md text-slate-300 hover:bg-slate-700 hover:text-white transition-colors text-xs flex items-center gap-2';
...
function buildFileList(
  ...
  filenames.forEach((name, i) => {
    const row = document.createElement('div');
    row.className =
      'flex items-center justify-between bg-slate-950 rounded-lg px-3 py-2';
    ...
    removeBtn.className =
      'text-slate-500 hover:text-red-400 text-lg leading-none flex-shrink-0';
...
function showNodeSettings(node: BaseWorkflowNode) {
  ...
  if (node instanceof PDFInputNode) {
    ...
    uploadBtn.className =
      'w-full bg-slate-700 hover:bg-slate-600 text-white text-xs px-3 py-2 rounded-lg transition-colors';
      ...
      for (const file of files) {
        try {
          ...
          if (err instanceof EncryptedPDFError) {
            ...
            showAlert('Hiba', 'Failed to load PDF: ' + (err as Error).message);
          }
        }
      }
      showNodeSettings(node);
  ...
  if (node instanceof ImageInputNode) {
    ...
    const label = document.createElement('label');
    label.className = 'block text-xs text-slate-400 mb-1';
    label.textContent = 'Képek';
    fileSection.appendChild(label);

    const formatHint = document.createElement('p');
    formatHint.className = 'text-[10px] text-slate-500 mb-2';
    formatHint.textContent =
      'Támogatott: JPG, PNG, BMP, GIF, TIFF, WebP, HEIC, PSD, SVG, PNM, PGM, PBM, PPM, PAM, JXR, JPX, JP2';
    ...
    fileInput.addEventListener('change', async (e) => {
      ...
      try {
        ...
        showAlert('Hiba', 'Failed to load images: ' + (err as Error).message);
      }
    });
  ...
  if (node instanceof DigitalSignNode) {
    ...
    certLabel.className = 'block text-xs text-slate-400 mb-1';
    certLabel.textContent = 'Tanúsítvány (.pfx, .p12, .pem)';
    certSection.appendChild(certLabel);

    if (node.hasCertFile()) {
      const certFileDiv = document.createElement('div');
      certFileDiv.className =
        'flex items-center justify-between bg-slate-700 px-3 py-2 rounded-lg mb-2';

      const certName = document.createElement('span');
      certName.className = 'text-xs text-slate-200 truncate flex-1';
      ...
      if (node.needsPassword()) {
        ...
        pwLabel.className = 'block text-xs text-slate-400 mb-1';
        pwLabel.textContent = 'Tanúsítvány jelszava';
        ...
        pwInput.placeholder = 'Add meg a jelszót...';
        pwInput.className =
          'flex-1 bg-slate-700 border border-slate-600 text-white text-xs px-3 py-2 rounded-lg focus:outline-none focus:border-accent';

        const unlockBtn = document.createElement('button');
        unlockBtn.className =
          'bg-accent hover:bg-accent text-white text-xs px-3 py-2 rounded-lg transition-colors flex-shrink-0';
        ...
        const doUnlock = async () => {
          ...
          if (!pw) return;
          unlockBtn.textContent = 'Feloldás folyamatban...';
          ...
          if (success) {
            ...
            statusMsg.textContent = 'Hibás jelszó';
        ...
        okMsg.textContent = 'A tanúsítvány feloldva';
    ...
    divider.className = 'border-t border-slate-700 my-3';
  ...
  if (fileInputConfig) {
    ...
    if (fileInputConfig.hint) {
      ...
      hint.className = 'text-[10px] text-slate-500 mb-2';
    ...
    fileInput.addEventListener('change', async (e) => {
      ...
      try {
        ...
        showAlert('Hiba', `Failed to load files: ${(err as Error).message}`);
      }
    });
  ...
  if (controlEntries.length === 0) {
    ...
    empty.textContent = 'Ehhez a csomóponthoz nincs beállítható opció.';
  ...
  const inputClass =
    'w-full bg-slate-950 border border-slate-600 text-white rounded-md px-2 py-1.5 text-xs focus:border-accent focus:outline-none';
  ...
  for (const [key, control] of controlEntries) {
    ...
    controlLabel.className = 'block text-xs text-slate-400 mb-1';
    ...
    if (dropdownOptions[key] && multiSelectDropdowns.has(key)) {
      ...
      searchInput.placeholder = 'Keresés a nyelvek között...';
      ...
      listDiv.className =
        'max-h-32 overflow-y-auto bg-slate-800 rounded border border-slate-600 mt-1';
      ...
      function updateTags() {
        ...
        for (const val of selectedValues) {
          ...
          tag.className =
            'inline-flex items-center gap-1 px-2 py-0.5 rounded bg-accent text-white text-[10px]';
      ...
      function renderList(filter: string) {
        ...
        for (const opt of dropdownOptions[key]) {
          ...
          const label = document.createElement('label');
          label.className =
            'flex items-center gap-2 px-2 py-1 hover:bg-slate-700 cursor-pointer text-xs text-slate-300';
          const cb = document.createElement('input');
          cb.type = 'checkbox';
          cb.checked = selectedValues.has(opt.value);
          cb.className =
            'w-3 h-3 rounded text-accent bg-slate-700 border-slate-600';
      ...
      toggle.className = `relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full transition-colors duration-200 ${isOn ? 'bg-accent' : 'bg-slate-600'}`;
      ...
      toggle.addEventListener('click', () => {
        ...
        toggle.className = `relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full transition-colors duration-200 ${on ? 'bg-accent' : 'bg-slate-600'}`;
  ...
  if (hasAdvanced && advancedWrappers.length > 0) {
    ...
    details.className =
      'bg-slate-800/50 border border-slate-700 rounded-lg p-2 mt-1';
    const summary = document.createElement('summary');
    summary.className =
      'text-xs font-medium text-slate-400 cursor-pointer select-none flex items-center justify-between';
    const summaryText = document.createElement('span');
    summaryText.textContent = 'Speciális beállítások';
    summary.appendChild(summaryText);
    const chevron = document.createElement('i');
    chevron.className =
      'ph ph-caret-down text-xs text-slate-500 transition-transform duration-200';
