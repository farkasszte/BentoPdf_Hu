...
function updateFileDisplay() {
  ...
  metaSpan.textContent = `${formatBytes(deleteState.file.size)} • ${deleteState.totalPages} oldal`;
...
async function renderThumbnails() {
  ...
  for (let i = 1; i <= deleteState.totalPages; i++) {
    ...
    imgContainer.className =
      'w-full h-28 bg-slate-950 rounded-lg flex items-center justify-center overflow-hidden border-2 border-slate-600';
    ...
    pageLabel.className =
      'absolute top-1 left-1 bg-slate-800 text-white text-xs px-1.5 py-0.5 rounded';
...
async function deletePages() {
  if (deleteState.pagesToDelete.size === 0) {
    showAlert('Nincs oldal', 'Válassz ki törlendő oldalakat.');
    return;
  }

  if (deleteState.pagesToDelete.size >= deleteState.totalPages) {
    showAlert('Hiba', 'Cannot delete all pages.');
    return;
  }
  ...
  try {
    ...
    hideLoader();
    showAlert('Hiba', 'Failed to delete pages.');
  }
}
