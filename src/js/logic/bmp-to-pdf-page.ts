...
async function convertImageToPngBytes(file: File): Promise<ArrayBuffer> {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => reject(new Error('A fájl olvasása nem sikerült.'));
...
const updateUI = () => {
  ...
  if (files.length > 0) {
    ...
    files.forEach((file, index) => {
      ...
      sizeSpan.className = 'flex-shrink-0 text-slate-400 text-xs';
...
async function convert() {
  if (files.length === 0) {
    showAlert('Nincs fájl', 'Please select at least one BMP file.');
    return;
  }
  ...
  try {
    ...
    showAlert('Siker', 'PDF created successfully!', 'success', () => {
      resetState();
    });
  } catch (e) {
    console.error(e);
    showAlert(
      'Hiba',
      'A BMP PDF-be konvertálása nem sikerült. Lehet, hogy az egyik fájl érvénytelen.'
    );
  } finally {
    hideLoader();
  }
