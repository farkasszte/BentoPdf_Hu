    ...
    const fieldsHTML = fields
      .map((field) => {
        if (field.type === 'text') {
          return `
  <div class="mb-4">
    <label class="block text-sm font-medium text-slate-700 mb-2">${escapeHTML(field.label)}</label>
      <input type="text" id="modal-${field.name}" value="${escapeHTML(String(defaultValues[field.name] || ''))}"
class="w-full px-3 py-2 border border-slate-300 rounded-lg text-gray-900"
placeholder="${escapeHTML(field.placeholder || '')}" />
  ...
  <div class="mb-4">
    <label class="block text-sm font-medium text-slate-700 mb-2">${escapeHTML(field.label)}</label>
      <select id="modal-${field.name}" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-gray-900">
        ${field.options
          .map(
            (opt) => `
        ...
        <div class="flex items-center gap-2">
          <label class="flex items-center gap-1 text-xs">
            <input type="checkbox" id="modal-use-destination" class="w-4 h-4" ${hasDestination ? 'checked' : ''}>
              <span class="text-slate-700">Set custom destination</span>
                </label>
                </div>
                <div id="destination-controls" class="${hasDestination ? '' : 'hidden'} space-y-2">
                  <div class="grid grid-cols-2 gap-2">
                    <div>
                    <label class="text-xs text-slate-600">Page</label>
...
class="w-full px-2 py-1 border border-slate-300 rounded text-sm text-gray-900" step="1" />
  </div>
  <div>
  <label class="text-xs text-slate-600">Zoom(%)</label>
    <select id="modal-dest-zoom" class="w-full px-2 py-1 border border-slate-300 rounded text-sm text-gray-900">
                ...
                <option value="125">125%</option>
                  <option value="150">150%</option>
                    <option value="200">200%</option>
                      ...
                      <div class="grid grid-cols-2 gap-2">
                        <div>
                        <label class="text-xs text-slate-600">X Position</label>
                          <input type="number" id="modal-dest-x" value="0" step="10"
class="w-full px-2 py-1 border border-slate-300 rounded text-sm text-gray-900" />
  </div>
  <div>
  <label class="text-xs text-slate-600">Y Position</label>
...
class="w-full px-2 py-1 border border-slate-300 rounded text-sm text-gray-900" />
  ...
  <button id="modal-pick-destination" class="w-full px-3 py-2 btn-gradient text-white rounded text-xs !flex items-center justify-center gap-1">
    <i data-lucide="crosshair" class="w-3 h-3"></i> Click on PDF to Pick Location
      ...
      <p class="text-xs text-slate-500 italic">Click the button above, then click on the PDF where you want the bookmark to jump to</p>
        </div>
        </div>
        </div>
          `;
        ...
        <div class="mb-4">
          <label class="block text-sm font-medium text-slate-700 mb-2">${escapeHTML(field.label)}</label>
            <div id="modal-preview" class="style-preview bg-slate-50">
    ...
    modal.innerHTML = `
                <div class="p-6">
                  <h3 class="text-xl font-bold text-slate-800 mb-4">${escapeHTML(title)}</h3>
                    <div class="mb-6">
                      ${fieldsHTML}
</div>
  <div class="flex gap-2 justify-end">
    <button id="modal-cancel" class="px-4 py-2 rounded-lg bg-slate-200 hover:bg-slate-300 text-slate-700">Cancel</button>
...
function showConfirmModal(message: string): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    modal.innerHTML = `
  <div class="p-6">
    <h3 class="text-xl font-bold text-slate-800 mb-4">Confirm Action</h3>
...
function handleResize(): void {
  if (window.innerWidth >= 1024) {
    ...
    showViewerBtn?.classList.remove('bg-accent', 'text-white');
    showViewerBtn?.classList.add('text-slate-300');
    showBookmarksBtn?.classList.remove('bg-accent', 'text-white');
    showBookmarksBtn?.classList.add('text-slate-300');
  }
}
...
showViewerBtn?.addEventListener('click', () => {
  ...
  showViewerBtn?.classList.add('bg-accent', 'text-white');
  showViewerBtn?.classList.remove('text-slate-300');
  showBookmarksBtn?.classList.remove('bg-accent', 'text-white');
  showBookmarksBtn?.classList.add('text-slate-300');
});
...
showBookmarksBtn?.addEventListener('click', () => {
  ...
  bookmarksSection?.classList.remove('hidden');
  showBookmarksBtn?.classList.add('bg-accent', 'text-white');
  showBookmarksBtn?.classList.remove('text-slate-300');
  showViewerBtn?.classList.remove('bg-accent', 'text-white');
  showViewerBtn?.classList.add('text-slate-300');
});
...
function renderFileDisplay(file: File): void {
  ...
  nameSpan.className = 'truncate font-medium text-slate-200';
  nameSpan.textContent = file.name;

  const sizeSpan = document.createElement('span');
  sizeSpan.className = 'flex-shrink-0 ml-4 text-slate-400';
...
jsonInput?.addEventListener('change', async (e: Event) => {
  ...
  try {
    ...
    await showAlertModal('Hiba', 'Invalid JSON format');
  }
});
...
async function renderPage(
  ...
  pageIndicator!.textContent = `${num}. oldal / ${pdfJsDoc.numPages}`;
...
function getTextColor(color: BookmarkColor | string): string {
  ...
  return TEXT_COLOR_CLASSES[color] || 'text-slate-700';
}
...
function createNodeElement(node: BookmarkNode, level = 0): HTMLLIElement {
  ...
  div.className = `flex items-center gap-2 p-2 rounded border border-slate-200 ${colorClass} ${highlight} ${isSelected ? 'ring-2 ring-blue-500' : ''} hover:bg-slate-50`;
  ...
  titleDiv.innerHTML = `
                ...
                <span class="text-xs text-slate-500">Page ${escapeHTML(String(node.page))}</span>
            `;
  ...
  addChildBtn.className = 'p-1 hover:bg-slate-200 rounded text-slate-700';
  ...
  editBtn.className = 'p-1 hover:bg-slate-200 rounded text-slate-700';
  editBtn.title = 'Edit';
  ...
  deleteBtn.className = 'p-1 hover:bg-slate-200 rounded text-red-600';
...
addTopLevelBtn?.addEventListener('click', async () => {
  ...
  if (!title) {
    await showAlertModal('Hiba', 'Please enter a title.');
    return;
  }
...
csvImportHidden?.addEventListener('change', async (e: Event) => {
  ...
  if (imported.length > 0) {
    ...
    saveState();
    renderBookmarkTree();
    await showAlertModal('Siker', `Imported ${imported.length} bookmarks!`);
  }
...
exportCsvBtn?.addEventListener('click', () => {
  ...
  if (bookmarkTree.length === 0) {
    showAlertModal('Hiba', 'No bookmarks to export!');
    return;
  }
...
jsonImportHidden?.addEventListener('change', async (e: Event) => {
  ...
  try {
    ...
    saveState();
    renderBookmarkTree();
    await showAlertModal('Siker', 'Bookmarks imported from JSON!');
  } catch {
    await showAlertModal('Hiba', 'Invalid JSON format');
  }
...
downloadBtn?.addEventListener('click', async () => {
  ...
  try {
    ...
    await showAlertModal('Siker', 'PDF saved successfully!');
