...
async function linearizePdf() {
  ...
  if (!pdfFiles || pdfFiles.length === 0) {
    showAlert('Nincs PDF-fájl', 'Tölts fel legalább egy PDF-fájlt.');
    return;
  }
  ...
  try {
    ...
    for (let i = 0; i < pdfFiles.length; i++) {
      ...
      if (loaderText)
        loaderText.textContent = `Optimalizálás: ${file.name} (${i + 1}/${pdfFiles.length})...`;
    ...
    showAlert(
      ...
      `An error occurred: ${error instanceof Error ? error.message : 'Ismeretlen hiba'}.`
    );
  } finally {
