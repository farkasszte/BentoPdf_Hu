...
async function handleFileUpload(file: File) {
  if (!file || file.type !== 'application/pdf') {
    showAlert('Hiba', 'Please upload a valid PDF file.');
    return;
  }
  try {
...
async function detectBlankPages() {
  if (!pageState.pdfDoc || !pageState.file)
    return showAlert('Hiba', 'Előbb tölts fel egy PDF-et.');
  ...
  try {
    ...
    console.error(e);
    showAlert('Hiba', 'Could not detect blank pages.');
    hideLoader();
  }
}
...
function updatePreviewPanel() {
  ...
  previewInfo.textContent = `${pageState.detectedBlankPages.length} üres oldal található. Kattints egy oldalra a kijelölés megszüntetéséhez.`;
  ...
  pageState.detectedBlankPages.forEach((pageIndex) => {
    ...
    div.className =
      'relative cursor-pointer flex flex-col items-center gap-1 p-2 border-2 border-red-500 rounded-lg bg-slate-700 transition-colors group';
    ...
    div.innerHTML = `
            <div class="relative">
                ...
                <div class="absolute top-1 left-1 bg-accent text-white text-xs px-2 py-1 rounded-md font-semibold shadow-lg z-10 pointer-events-none">
                    ${pageIndex + 1}
                </div>
...
function togglePageSelection(div: HTMLElement, pageIndex: number) {
  ...
  if (isSelected) {
    ...
    div.classList.add('border-slate-600', 'opacity-50');
    checkMark?.classList.add('hidden');
  } else {
    div.dataset.selected = 'true';
    div.classList.add('border-red-500');
    div.classList.remove('border-slate-600', 'opacity-50');
...
async function processRemoveBlankPages() {
  ...
  try {
    ...
    console.error(e);
    showAlert('Hiba', 'Could not remove blank pages.');
  } finally {
    hideLoader();
  }
}
