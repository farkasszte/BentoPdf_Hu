...
export async function repairPdf() {
  if (state.files.length === 0) {
    showAlert('Nincs fájl', 'Please select one or more PDF files.');
    return;
  }
  ...
  try {
    ...
    if (failedRepairs.length > 0) {
      ...
      showAlert(
        'Partial Success',
        `Sikeresen javítva: ${successfulRepairs.length} fájl. Sikertelen javítás: ${failedList}`
      );
    }
    ...
    if (failedRepairs.length === 0) {
      showAlert('Siker', 'All files repaired successfully!');
    }
  } catch (error: unknown) {
