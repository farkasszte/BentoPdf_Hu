...
async function renderPageMergeThumbnails() {
  ...
  try {
    ...
    showAlert('Hiba', 'Failed to render page thumbnails');
  } finally {
    hideLoader();
...
export async function merge() {
  ...
  try {
    ...
    if (jobs.length === 0) {
      showAlert('Hiba', 'No files or pages selected to merge.');
      hideLoader();
      return;
    }
    ...
    mergeWorker.onerror = (e) => {
      ...
      showAlert('Hiba', 'An unexpected error occurred in the merge worker.');
    };
  } catch (e) {
...
export async function refreshMergeUI() {
  ...
  try {
    ...
    showAlert('Hiba', 'Failed to load one or more PDF files');
    return;
  } finally {
    hideLoader();
  }
