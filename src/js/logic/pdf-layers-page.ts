...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateUI = async () => {
    ...
    if (currentFile) {
      ...
      try {
        ...
        metaSpan.textContent = `${formatBytes(currentFile.size)} • ${pdfDoc.numPages} oldal`;
      } catch (error) {
        console.error('Error loading PDF:', error);
        metaSpan.textContent = `${formatBytes(currentFile.size)} • Az oldalszám betöltése nem sikerült`;
      }
  ...
  const renderLayers = () => {
    ...
    if (layersArray.length === 0) {
      layersList.innerHTML = `
                <div class="layers-empty">
                    <p>Ennek a PDF-nek nincsenek rétegei (OCG).</p>
                    <p>Adj hozzá egy új réteget a kezdéshez!</p>
                </div>
            `;
      return;
    }
      ...
      .forEach((checkbox) => {
        checkbox.addEventListener('change', (e) => {
          ...
          try {
            ...
            target.checked = !isOn;
            showAlert('Hiba', 'Failed to toggle layer visibility');
          }
        });
      });
    ...
    layersList.querySelectorAll('.layer-delete').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        ...
        if (!layer) {
          showAlert('Hiba', 'Layer not found');
          return;
        }
        ...
        try {
          ...
          showAlert('Hiba', 'Failed to delete layer');
        }
      });
    });
    ...
    layersList.querySelectorAll('.layer-add-child').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        try {
          ...
          showAlert('Hiba', 'Failed to add child layer');
        }
      });
    });
  };
  ...
  const loadLayers = async () => {
    if (!currentFile) {
      showAlert('Nincs fájl', 'Please select a PDF file.');
      return;
    }
    ...
    try {
      ...
      showLoader(`Rétegek betöltése innen: ${currentFile.name}...`);
      currentDoc = await (
    ...
    if (saveLayersBtn) {
      saveLayersBtn.onclick = () => {
        try {
          ...
          hideLoader();
          resetState();
          showAlert('Siker', 'PDF with layer changes saved!', 'success');
        } catch (err: unknown) {
          hideLoader();
          showAlert(
            'Hiba',
