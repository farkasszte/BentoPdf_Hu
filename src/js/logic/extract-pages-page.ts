...
function updateFileDisplay() {
  ...
  metaSpan.textContent = `${formatBytes(extractState.file.size)} • ${extractState.totalPages} oldal`;
...
async function extractPages() {
  ...
  if (!pagesInput || !pagesInput.value.trim()) {
    showAlert('Nincs oldal', 'Add meg a kinyerendő oldalszámokat.');
    return;
  }
  ...
  try {
    ...
    hideLoader();
    showAlert('Hiba', 'Failed to extract pages.');
  }
}
