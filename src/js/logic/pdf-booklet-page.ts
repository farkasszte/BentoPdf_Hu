...
function resetState() {
  ...
  if (previewArea)
    previewArea.innerHTML =
      '<p class="text-slate-400 text-center py-8">Upload a PDF and click "Generate Preview" to see the booklet layout</p>';
...
async function generatePreview() {
  if (!pageState.pdfDoc || !pageState.pdfjsDoc) {
    showAlert('Hiba', 'Please load a PDF first.');
    return;
  }
  ...
  previewArea.innerHTML =
    '<p class="text-slate-400 text-center py-4">Generating preview...</p>';
  ...
  previewArea.innerHTML = `<p class="text-accent text-sm mb-4 text-center">${totalPages} pages → ${numSheets} output sheets</p>`;

  for (let sheetIndex = 0; sheetIndex < numSheets; sheetIndex++) {
    const canvas = document.createElement('canvas');
    canvas.width = canvasWidth;
    canvas.height = canvasHeight;
    canvas.className = 'border border-slate-600 rounded-lg mb-4';
...
async function createBooklet() {
  ...
  try {
    ...
    console.error(e);
    showAlert('Hiba', 'An error occurred while creating the booklet.');
  } finally {
    hideLoader();
  }
}
