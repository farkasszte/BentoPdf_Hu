...
async function decryptPdf() {
  if (pageState.files.length === 0) {
    showAlert('Nincs fájl', 'Tölts fel legalább egy PDF-fájlt.');
    return;
  }
  ...
  try {
    ...
    if (loaderText) loaderText.textContent = 'Visszafejtés előkészítése...';

    if (pageState.files.length === 1) {
      // Single file: decrypt and download directly
      const file = pageState.files[0];
      if (loaderText) loaderText.textContent = 'Titkosított PDF olvasása...';
      ...
      for (let i = 0; i < pageState.files.length; i++) {
        ...
        if (loaderText)
          loaderText.textContent = `Visszafejtés: ${file.name} (${i + 1}/${pageState.files.length})...`;
