...
worker.onmessage = function (e) {
  ...
  if (data.status === 'success' && data.attachments !== undefined) {
    ...
    hideLoader();
    showAlert('Hiba', data.message || 'Ismeretlen hiba történt.');
  }
};
...
worker.onerror = function (error) {
  ...
  showAlert('Hiba', 'Worker error occurred. Check console for details.');
};
...
function displayAttachments(attachments: AttachmentInfo[]) {
  ...
  if (attachments.length === 0) {
    ...
    noAttachments.className = 'text-slate-400 text-center py-4';
    noAttachments.textContent = 'Ebben a PDF-ben nincs melléklet.';
  ...
  for (const attachment of attachments) {
    ...
    attachmentDiv.className =
      'flex items-center justify-between p-3 bg-slate-800 rounded-lg border border-slate-700';
    ...
    levelSpan.className = 'text-slate-400 text-sm block';
    if (attachment.page === 0) {
      levelSpan.textContent = 'Dokumentumszintű melléklet';
    } else {
      levelSpan.textContent = `${attachment.page}. oldal melléklete`;
    }
    ...
    removeBtn.className = `${pageState.attachmentsToRemove.has(attachment.index) ? 'bg-slate-600' : 'bg-red-600'} hover:bg-red-700 text-white px-3 py-1 rounded text-sm`;
...
async function loadAttachments() {
  ...
  showLoader('Mellékletek betöltése...');
  ...
  try {
    ...
    hideLoader();
    showAlert('Hiba', 'Failed to load attachments from PDF.');
  }
}

async function saveChanges() {
  if (!pageState.file) {
    showAlert('Hiba', 'No PDF file loaded.');
    return;
  }
  ...
  try {
    ...
    hideLoader();
    showAlert('Hiba', 'Failed to edit attachments.');
  }
}
