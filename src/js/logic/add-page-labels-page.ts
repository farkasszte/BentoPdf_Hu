...
async function handleFiles(files: FileList) {
  ...
  if (
    ...
    showAlert(
      translate('tools:addPageLabels.invalidFileTitle', 'Érvénytelen fájl'),
      translate(
  ...
  showLoader(translate('tools:addPageLabels.loadingPdf', 'PDF betöltése...'));
  try {
    ...
    console.error(error);
    showAlert(
      translate('common.error', 'Hiba'),
      translate(
        'tools:addPageLabels.loadErrorMessage',
        'A PDF-fájl betöltése nem sikerült. A fájl érvénytelen, sérült vagy jelszóval védett lehet.'
      )
    );
  } finally {
    hideLoader();
...
function updateFileDisplay() {
  ...
  metaSpan.textContent = translate(
    ...
    `${formatBytes(pageState.file.size)} • ${pageState.pageCount} oldal`,
    {
...
function renderRules() {
  ...
  pageState.rules.forEach((rule, index) => {
    ...
    card.className =
      'rounded-lg border border-slate-700 bg-slate-950 p-4 space-y-4';
    ...
    removeBtn.className =
      'text-red-400 hover:text-red-300 disabled:text-slate-600 disabled:cursor-not-allowed';
    ...
    rangeLabel.className = 'block mb-2 text-sm font-medium text-slate-300';
    ...
    rangeInput.className =
      'w-full bg-slate-700 border border-slate-600 text-white rounded-lg p-2.5 focus:ring-accent focus:border-accent';
    ...
    styleLabel.className = 'block mb-2 text-sm font-medium text-slate-300';
    ...
    prefixLabel.className = 'block mb-2 text-sm font-medium text-slate-300';
    ...
    startLabel.className = 'block mb-2 text-sm font-medium text-slate-300';
    ...
    progressLabel.className =
      'flex w-full items-center gap-3 rounded-lg border border-slate-700 bg-slate-800 px-4 py-3 text-sm text-slate-300';
    const progressInput = document.createElement('input');
    progressInput.type = 'checkbox';
    progressInput.checked = rule.progress;
    progressInput.className =
      'h-4 w-4 rounded border-slate-500 bg-slate-700 text-accent focus:ring-accent';
...
async function addPageLabels() {
  ...
  try {
    ...
    showAlert(
      translate('common.success', 'Siker'),
      translate(
    ...
    const message =
      error instanceof Error
        ...
        : translate(
            ...
            'Az oldalcímkék hozzáadása nem sikerült.'
          );
    showAlert(translate('common.error', 'Hiba'), message);
  } finally {
    if (cpdf && pdf) {
      try {
        cpdf.deletePdf(pdf);
