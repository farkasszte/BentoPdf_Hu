...
async function updateFileDisplay() {
  ...
  metaSpan.textContent = `${formatBytes(result.file.size)} • ${result.pdf.numPages} oldal`;
  result.pdf.destroy();
}
...
async function setupSignTool() {
  ...
  showLoader('PDF-megjelenítő betöltése...');
...
async function applyAndSaveSignatures() {
  ...
  try {
    ...
    if (shouldFlatten) {
      ...
      hideLoader();
      showAlert('Siker', 'Signed PDF saved successfully!', 'success', () => {
        resetState();
      });
    } else {
    ...
    showAlert(
      'Export failed',
      'Az aláírt PDF exportálása nem sikerült. Próbáld újra.'
    );
  }
}
