...
async function handleFileInput(
  ...
  async function handleFile(file: File) {
    if (!file || file.type !== 'application/pdf') {
      showAlert('Érvénytelen fájl', 'Please select a valid PDF file.');
      return;
    }
    ...
    try {
      ...
      if (!result) return;
      showLoader(`Betöltés: ${result.file.name}...`);
      ...
      showAlert(
        'Hiba',
        'A PDF betöltése nem sikerült. Lehet, hogy sérült vagy jelszóval védett.'
      );
      console.error(e);
    } finally {
      hideLoader();
