...
async function processAllPages(): Promise<void> {
  ...
  try {
    ...
    showAlert(
      'Hiba',
      'A szkennelési effektus alkalmazása nem sikerült. A fájl sérült lehet.'
    );
  } finally {
    hideLoader();
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const handleFileSelect = async (newFiles: FileList | null) => {
    ...
    if (validFiles.length === 0) {
      showAlert('Érvénytelen fájl', 'Tölts fel egy PDF-fájlt.');
      return;
    }
    ...
    try {
      ...
      console.error(e);
      showAlert('Hiba', 'A PDF betöltése az előnézethez nem sikerült.');
    } finally {
      hideLoader();
    }
  };
