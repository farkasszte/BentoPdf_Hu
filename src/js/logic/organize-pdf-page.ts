...
function applyCustomOrder() {
  ...
  showAlert('Siker', 'Pages have been reordered.', 'success');
}
...
async function saveChanges() {
  ...
  try {
    ...
    if (finalIndices.length === 0) {
      showAlert('Hiba', 'No valid pages to save.');
      return;
    }
    ...
    hideLoader();
    showAlert('Siker', 'PDF organized successfully!', 'success', () =>
      resetState()
    );
  } catch (error) {
    console.error('Error saving changes:', error);
    hideLoader();
    showAlert('Hiba', 'Failed to save changes.');
  }
}
