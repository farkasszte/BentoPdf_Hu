...
async function handleFiles(files: FileList) {
  ...
  if (!file || file.type !== 'application/pdf') {
    showAlert('Érvénytelen fájl', 'Please upload a valid PDF file.');
    return;
  }
  try {
    ...
    if (editorFileInfo) {
      editorFileInfo.textContent = `${file.name} (${formatBytes(file.size)}, ${totalPageCount} oldal)`;
    }
...
function updateFileDisplay() {
  ...
  metaSpan.textContent = `${formatBytes(pageState.file.size)} • ${pageState.pdfDoc.getPageCount()} oldal`;
...
function loadPageConfig(pageNum: number) {
  ...
  if (config.type === 'text') {
    typeTextBtn!.className =
      'flex-1 py-2 px-3 text-sm font-medium rounded-lg bg-accent text-white transition-colors';
    typeImageBtn!.className =
      'flex-1 py-2 px-3 text-sm font-medium rounded-lg bg-slate-700 text-slate-300 hover:bg-slate-600 transition-colors';
...
function updatePresetHighlight(x: number, y: number) {
  document.querySelectorAll('.pos-preset-btn').forEach((btn) => {
    ...
    if (Math.abs(bx - x) < 0.01 && Math.abs(by - y) < 0.01) {
      btn.className =
        'pos-preset-btn py-1.5 text-xs bg-accent hover:bg-accent text-white rounded-md transition-colors';
    } else {
      btn.className =
        'pos-preset-btn py-1.5 text-xs bg-slate-700 hover:bg-slate-600 text-slate-300 rounded-md transition-colors';
    }
  });
}
...
async function applyWatermark() {
  if (!pageState.pdfDoc || !pageState.pdfBytes) {
    showAlert('Hiba', 'Előbb tölts fel egy PDF-fájlt.');
    return;
  }
  ...
  try {
    ...
    if (applyToAllPages) {
      ...
      if (config.type === 'text') {
        ...
        if (!imageFile) throw new Error('Válassz ki egy képet a vízjelhez.');
    ...
    showAlert('Siker', 'A vízjel sikeresen hozzáadva!', 'success');
  } catch (e: unknown) {
    console.error(e);
    showAlert(
      'Hiba',
      e instanceof Error ? e.message : 'A vízjel hozzáadása nem sikerült.'
    );
  } finally {
    hideLoader();
  }
