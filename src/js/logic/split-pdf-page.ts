...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (state.files.length > 0) {
      ...
      if (fileDisplayArea) {
        ...
        try {
          ...
          metaSpan.textContent = `${formatBytes(file.size)} • ${pageCount} oldal`;
        } catch (error) {
          console.error('Error loading PDF:', error);
          showAlert('Hiba', 'A PDF-fájl betöltése nem sikerült.');
          state.files = [];
          updateUI();
          return;
        }
  ...
  const renderVisualSelector = async () => {
    ...
    try {
      ...
      const createWrapper = (canvas: HTMLCanvasElement, pageNumber: number) => {
        ...
        wrapper.className =
          'page-thumbnail-wrapper p-2 border-2 border-slate-600 rounded-lg cursor-pointer hover:border-accent bg-slate-700 transition-colors relative group flex flex-col items-center gap-1';
        ...
        const handleSelection = (e: Event) => {
          ...
          if (isSelected) {
            wrapper.classList.remove('selected', 'border-accent');
            wrapper.classList.add('border-slate-600');
          } else {
            wrapper.classList.add('selected', 'border-accent');
            wrapper.classList.remove('border-slate-600');
          }
        };
      ...
      showAlert('Hiba', 'Failed to render page previews.');
  ...
  const resetState = () => {
    ...
    document
      ...
      .forEach((el) => {
        el.classList.remove('selected', 'border-accent');
    ...
    try {
      ...
      switch (splitMode) {
        ...
        case 'bookmarks': {
          ...
          hideLoader();
          showAlert('Siker', 'PDF split successfully!', 'success', () => {
            resetState();
          });
          return;
        }
    ...
    splitModeSelect.addEventListener('change', (e) => {
      ...
      if (mode === 'range') {
        ...
        if (outputSeparateLabel)
          outputSeparateLabel.textContent = 'Egy PDF tartományonként';
      } else if (mode === 'visual') {
        visualPanel?.classList.remove('hidden');
        outputModeWrapper?.classList.remove('hidden');
        if (outputSeparateLabel)
          outputSeparateLabel.textContent = 'Egy PDF oldalanként';
        ...
        const updateWarning = () => {
          ...
          if (remainder !== 0 && nTimesWarning) {
            ...
            if (warningText) {
              warningText.textContent = `A PDF összesen ${totalPages} oldalt tartalmaz, ami nem osztható egyenlően a következővel: ${nValue}. Az utolsó PDF ${remainder} oldalt fog tartalmazni.`;
            }
