...
async function handleFiles(fileList: FileList) {
  ...
  try {
    ...
    if (files.length === 0) {
      showAlert('Érvénytelen fájl', 'Please upload valid PDF files.');
      return;
    }
    ...
    console.error(error);
    showAlert('Hiba', 'Egy vagy több PDF-fájl betöltése nem sikerült.');
  } finally {
    hideLoader();
  }
}
...
function renderFileList() {
  ...
  files.forEach((entry, index) => {
    ...
    dragHandle.className =
      'drag-handle w-4 h-4 text-slate-400 cursor-grab flex-shrink-0';

    const infoContainer = document.createElement('div');
    infoContainer.className = 'flex flex-col min-w-0';

    const nameSpan = document.createElement('div');
    nameSpan.className = 'truncate font-medium text-slate-200 text-sm';
    nameSpan.textContent = entry.file.name;

    const metaSpan = document.createElement('div');
    metaSpan.className = 'text-xs text-slate-400';
    metaSpan.textContent = `${formatBytes(entry.file.size)} \u2022 ${entry.pageCount} oldal`;
  ...
  summary.className = 'text-xs text-slate-400 mt-1';
  summary.textContent = `${files.length} fájl • összesen ${totalPages} oldal`;
...
async function applyBatesNumbers() {
  if (files.length === 0) {
    showAlert('Hiba', 'Tölts fel legalább egy PDF-fájlt.');
    return;
  }
  ...
  try {
    ...
    console.error(e);
    showAlert('Hiba', 'Failed to apply Bates numbers.');
  } finally {
    hideLoader();
  }
}
