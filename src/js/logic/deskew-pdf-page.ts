...
function updateFileDisplay(): void {
  ...
  selectedFiles.forEach((file, index) => {
    ...
    nameSpan.className = 'text-slate-200 truncate max-w-xs';
    nameSpan.textContent = file.name;

    const sizeSpan = document.createElement('span');
    sizeSpan.className = 'text-slate-500 text-sm';
    sizeSpan.textContent = `(${(file.size / 1024).toFixed(1)} KB)`;

    info.append(fileIcon, nameSpan, sizeSpan);

    const removeBtn = document.createElement('button');
    removeBtn.className = 'remove-file text-slate-400 hover:text-red-400';
...
function displayResults(result: DeskewResult): void {
  ...
  anglesList.innerHTML = result.angles
    .map((angle, idx) => {
      ...
      const color = wasCorrected ? 'text-green-400' : 'text-slate-400';
      const icon = wasCorrected ? 'check' : 'minus';
      return `
        <div class="flex items-center gap-2 text-sm py-1">
          <i data-lucide="${icon}" class="w-4 h-4 ${color}"></i>
          <span class="text-slate-300">Page ${idx + 1}:</span>
...
async function processDeskew(): Promise<void> {
  ...
  try {
    ...
    showAlert(
      'Hiba',
      `Failed to deskew PDF: ${error instanceof Error ? error.message : 'Ismeretlen hiba'}`
    );
  }
}
