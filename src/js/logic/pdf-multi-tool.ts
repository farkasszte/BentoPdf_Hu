...
function showDragOverlay() {
  ...
  dragOverlay.className =
    'fixed inset-0 bg-accent/40 border-4 border-dashed border-accent-light z-40 pointer-events-none flex items-center justify-center';
  const inner = document.createElement('div');
  inner.className =
    'text-white text-lg sm:text-xl font-semibold bg-slate-950/80 px-6 py-4 rounded-lg text-center';
...
function createPageElement(
  ...
  card.className =
    'bg-slate-800 rounded-lg border-2 border-slate-700 p-2 relative group cursor-move';
  ...
  if (selectedPages.has(index)) {
    card.classList.add('border-accent', 'ring-2', 'ring-accent');
  }
  ...
  if (canvas) {
    ...
    preview.classList.add('bg-slate-700'); // Darker background for loading
  }

  // Page info
  const info = document.createElement('div');
  info.className = 'text-xs text-slate-400 text-center mb-2';
  ...
  actionsInner.className =
    'flex items-center gap-1 bg-slate-950/90 rounded px-2 py-1';
  actions.appendChild(actionsInner);

  // Select checkbox
  const selectBtn = document.createElement('button');
  selectBtn.className =
    'absolute top-2 right-2 p-1 rounded bg-slate-950/70 hover:bg-slate-800 z-10';
  const selectIcon = document.createElement('i');
  if (selectedPages.has(index)) {
    selectIcon.dataset.lucide = 'check-square';
    selectIcon.className = 'w-4 h-4 text-accent';
  } else {
    selectIcon.dataset.lucide = 'square';
    selectIcon.className = 'w-4 h-4 text-slate-200';
  }
  ...
  rotateBtn.className = 'p-1 rounded hover:bg-slate-700';
  rotateBtn.innerHTML =
    '<i data-lucide="rotate-cw" class="w-4 h-4 text-slate-300"></i>';
  rotateBtn.onclick = (e) => {
    e.stopPropagation();
    rotatePage(index, 90);
  };
  const rotateLeftBtn = document.createElement('button');
  rotateLeftBtn.className = 'p-1 rounded hover:bg-slate-700';
  rotateLeftBtn.innerHTML =
    '<i data-lucide="rotate-ccw" class="w-4 h-4 text-slate-300"></i>';
  ...
  duplicateBtn.className = 'p-1 rounded hover:bg-slate-700';
  ...
  deleteBtn.className = 'p-1 rounded hover:bg-slate-700';
  deleteBtn.innerHTML =
  ...
  insertBtn.className = 'p-1 rounded hover:bg-slate-700';
  insertBtn.innerHTML =
    '<i data-lucide="file-plus" class="w-4 h-4 text-slate-300"></i>';
  ...
  splitBtn.className = 'p-1 rounded hover:bg-slate-700';
  splitBtn.innerHTML =
...
function toggleSelectOptimized(index: number) {
  ...
  if (selectedPages.has(index)) {
    ...
    card.classList.remove('border-accent', 'ring-2', 'ring-accent');
    selectBtn.innerHTML =
      '<i data-lucide="square" class="w-4 h-4 text-slate-200"></i>';
  }
  ...
  try {
    ...
    for (let i = 0; i < numPages; i++) {
      ...
      if (allPages[globalIndex]) {
        ...
        if (card) {
          const preview =
            card.querySelector('.bg-slate-700') ||
    ...
    showModal(
      'Hiba',
      'A PDF beszúrása nem sikerült. A fájl sérült lehet.',
      'error'
    );
  }
...
function bulkRotate(delta: number) {
  if (selectedPages.size === 0) {
    showModal('Nincs kijelölés', 'Válassz ki forgatandó oldalakat.', 'info');
    return;
  }
...
function bulkDelete() {
  if (selectedPages.size === 0) {
    showModal('Nincs kijelölés', 'Válassz ki törlendő oldalakat.', 'info');
    return;
  }
...
function bulkDuplicate() {
  if (selectedPages.size === 0) {
    showModal('Nincs kijelölés', 'Válassz ki másolandó oldalakat.', 'info');
    return;
  }
  const indices = Array.from(selectedPages).sort((a, b) => b - a);
...
async function downloadAll() {
  if (allPages.length === 0) {
    showModal('Nincs oldal', 'Előbb tölts fel PDF-eket.', 'info');
    return;
  }
...
async function downloadSplitPdfs() {
  try {
    ...
    showModal(
      'Hiba',
      'A szétválasztott PDF-ek létrehozása nem sikerült.',
      'error'
    );
  } finally {
...
async function downloadPagesAsPdf(indices: number[], filename: string) {
  try {
    ...
    showModal('Siker', 'A PDF sikeresen letölthető.', 'success');
  } catch (e) {
    console.error('Failed to create PDF:', e);
    showModal('Hiba', 'A PDF létrehozása nem sikerült.', 'error');
  }
}
  ...
  allPages.forEach((pageData, index) => {
    ...
    if (card) {
      ...
      const actionsInner = card.querySelector(
        '.flex.items-center.gap-1.bg-slate-950\\/90'
      );
      if (actionsInner) {
...
function updatePageNumbers() {
  ...
  cards.forEach((card, index) => {
    ...
    const info = card.querySelector('.text-xs.text-slate-400.text-center.mb-2');
    if (info) {
