...
function updateFileList() {
  ...
  sizeSpan.className = 'text-slate-400 text-sm';
  ...
  try {
    ...
    const initialize = async () => {
      try {
        ...
        if (eventBus && typeof eventBus._on === 'function') {
          eventBus._on('annotationeditoruimanager', () => {
            try {
              ...
              console.warn(
                'A bélyegzőgomb automatikus megnyomása nem sikerült a jegyzetelő szerkesztőben',
                e
              );
            }
          });
        ...
        console.error(
          'A jegyzetelő megjelenítő inicializálása nem sikerült a bélyegzőkhöz:',
          e
        );
      }
    };
...
if (saveStampedBtn) {
  saveStampedBtn.addEventListener('click', () => {
    ...
    try {
      ...
      alert(
        'A bélyegzett PDF exportálója nem érhető el. Használd az Export → PDF gombot a megjelenítő eszköztárában.'
      );
    } catch (e) {
      console.error('Failed to trigger stamped PDF export:', e);
      alert(
        'A bélyegzett PDF exportálása nem sikerült. Használd az Export → PDF gombot a megjelenítő eszköztárában.'
      );
    }
  });
}
