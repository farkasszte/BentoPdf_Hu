...
async function processFiles() {
  ...
  try {
    if (pageState.files.length === 1) {
      ...
      if (loaderText)
        loaderText.textContent = 'Betűtípusok körvonallá alakítása...';
      ...
      for (let i = 0; i < pageState.files.length; i++) {
        ...
        if (loaderText)
          loaderText.textContent = `Feldolgozás (${i + 1}/${pageState.files.length}): ${file.name}...`;
      ...
      if (processedCount > 0) {
        ...
        showAlert('Hiba', 'No PDFs could be processed.');
      }
      if (loaderModal) loaderModal.classList.add('hidden');
    }
  } catch (e: unknown) {
