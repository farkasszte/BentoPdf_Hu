...
function initializePage(): void {
  ...
  if (certDropZone) {
    certDropZone.addEventListener('dragover', (e) => {
      e.preventDefault();
      certDropZone.classList.add('bg-slate-700');
    });

    certDropZone.addEventListener('dragleave', () => {
      certDropZone.classList.remove('bg-slate-700');
    });
...
async function handlePdfFile(file: File): Promise<void> {
  if (
    ...
    showAlert('Érvénytelen fájl', 'Please select a PDF file.');
    return;
  }
...
function displayResults(): void {
  ...
  if (state.results.length === 0) {
    resultsContainer.innerHTML = `
            <div class="bg-slate-700 rounded-lg p-6 text-center border border-slate-600">
                <i data-lucide="file-x" class="w-12 h-12 mx-auto mb-4 text-slate-400"></i>
                <h3 class="text-lg font-semibold text-white mb-2">No Signatures Found</h3>
                <p class="text-slate-400">This PDF does not contain any digital signatures.</p>
            </div>
        `;
    createIcons({ icons });
    return;
  ...
  summaryDiv.className =
    'mb-4 p-3 bg-slate-700 rounded-lg border border-slate-600';
...
function createSignatureCard(
  ...
  card.className = 'bg-slate-700 rounded-lg p-4 border border-slate-600 mb-4';
  ...
  if (!result.isValid) {
    if (result.cryptoVerificationStatus === 'unsupported') {
      ...
      statusIcon = 'x-circle';
      statusText = 'Érvénytelen – aláírás után módosítva (részben fedett)';
    } else {
      statusColor = 'text-red-400';
      statusIcon = 'x-circle';
      statusText =
  ...
  if (state.trustedCert) {
    if (result.isTrusted) {
      ...
      trustBadge =
        '<span class="text-xs bg-slate-600 text-slate-300 px-2 py-1 rounded ml-2"><i data-lucide="shield-x" class="inline w-3 h-3 mr-1"></i>Not in trust chain</span>';
    }
  }
  ...
  card.innerHTML = `
        ...
        <div class="space-y-3 text-sm">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <p class="text-slate-400">Signed By</p>
                    <p class="text-white font-medium">${escapeHtml(result.signerName)}</p>
                    ${result.signerOrg ? `<p class="text-slate-400 text-xs">${escapeHtml(result.signerOrg)}</p>` : ''}
                    ${result.signerEmail ? `<p class="text-slate-400 text-xs">${escapeHtml(result.signerEmail)}</p>` : ''}
                </div>
                <div>
                    <p class="text-slate-400">Issuer</p>
                    <p class="text-white font-medium">${escapeHtml(result.issuer)}</p>
                    ${result.issuerOrg ? `<p class="text-slate-400 text-xs">${escapeHtml(result.issuerOrg)}</p>` : ''}
                </div>
            </div>
            ...
            ${
              result.signatureDate
                ? `
                <div>
                    <p class="text-slate-400">Signed On</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <p class="text-slate-400">Valid From</p>
            ...
            <details class="mt-2">
                <summary class="cursor-pointer text-accent hover:text-accent-light text-sm">
                    Technical Details
                </summary>
                <div class="mt-2 p-3 bg-slate-800 rounded text-xs space-y-1">
                    <p><span class="text-slate-400">Serial Number:</span> <span class="text-slate-300 font-mono">${escapeHtml(result.serialNumber)}</span></p>
                    <p><span class="text-slate-400">Digest Algorithm:</span> <span class="text-slate-300">${escapeHtml(result.algorithms.digest)}</span></p>
                    <p><span class="text-slate-400">Signature Algorithm:</span> <span class="text-slate-300">${escapeHtml(result.algorithms.signature)}</span></p>
