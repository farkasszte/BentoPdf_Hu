...
function updateFileDisplay() {
  ...
  metaSpan.textContent = `${formatBytes(cropperState.file.size)} • ${cropperState.pdfDoc?.numPages || 0} oldal`;
...
async function displayPageAsImage(num: number) {
  ...
  try {
    ...
    showAlert('Hiba', 'Failed to render page.');
    hideLoader();
  }
}
...
function updatePageInfo() {
  ...
  if (pageInfo)
    pageInfo.textContent = `${cropperState.currentPageNum}. oldal, összesen ${cropperState.pdfDoc.numPages}`;
}
...
async function performCrop() {
  ...
  if (isApplyToAll) {
    ...
    if (!currentCrop) {
      showAlert('Nincs vágási terület', 'Előbb válassz ki egy területet a vágáshoz.');
      return;
    }
  ...
  try {
    ...
    console.error(e);
    showAlert('Hiba', 'An error occurred during cropping.');
  } finally {
    hideLoader();
  }
}
