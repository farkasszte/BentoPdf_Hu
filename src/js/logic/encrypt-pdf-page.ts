...
async function encryptPdf() {
  if (!pageState.file) {
    showAlert('Nincs fájl', 'Előbb tölts fel egy PDF-fájlt.');
    return;
  }
  ...
  try {
    ...
    if (loaderText) loaderText.textContent = 'Titkosítás előkészítése...';
    ...
    if (loaderText)
      loaderText.textContent = 'PDF titkosítása 256 bites AES-sel...';
    ...
    try {
      ...
      throw new Error(
        'Encryption failed: ' +
          (qpdfError instanceof Error ? qpdfError.message : 'Ismeretlen hiba'),
        { cause: qpdfError }
      );
    }
    ...
    showAlert('Siker', successMessage, 'success', () => {
      resetState();
    });
  } catch (error: unknown) {
