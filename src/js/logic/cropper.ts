...
async function displayPageAsImage(num: number) {
  ...
  try {
    ...
    image.onload = () => {
      ...
      enableControls();
      hideLoader();
      showAlert('Kész', 'Rajzolj be egy területet a vágáshoz.');
    };
  } catch (error) {
...
function updatePageInfo() {
  document.getElementById('page-info').textContent =
    `${cropperState.currentPageNum}. oldal, összesen ${cropperState.pdfDoc.numPages}`;
}
...
export async function setupCropperTool() {
  ...
  try {
    ...
    showAlert('Hiba', 'Failed to load PDF for cropping.');
  }
  ...
  document.getElementById('crop-button').addEventListener('click', async () => {
    ...
    try {
      ...
      showAlert('Siker', 'Crop complete! Your download has started.');
    } catch (e) {
      console.error(e);
