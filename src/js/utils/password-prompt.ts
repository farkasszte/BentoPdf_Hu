...
export async function promptAndDecryptFile(file: File): Promise<File | null> {
  ...
  if (titleEl) titleEl.textContent = 'Jelszó szükséges';
  ...
  const modalPromise = new Promise<File | null>((resolve) => {
    ...
    async function onSubmit() {
      ...
      if (!password) {
        if (errorEl) {
          errorEl.textContent = 'Adj meg egy jelszót';
      ...
      if (progressEl) {
        progressEl.textContent = 'Ellenőrzés...';
      ...
      if (!valid) {
        ...
        input.focus();
        if (errorEl) {
          errorEl.textContent = 'Hibás jelszó. Próbáld újra.';
          errorEl.classList.remove('hidden');
        }
        return;
      }

      if (progressEl) progressEl.textContent = 'Visszafejtés...';
...
export async function promptAndDecryptBatch(
  ...
  if (titleEl)
    titleEl.textContent = `${fileNames.length} fájlhoz jelszó szükséges`;
  ...
  const batchPromise = new Promise<Map<number, File>>((resolve) => {
    ...
    function updateButtons(autoClose = false) {
      ...
      if (hasSucceeded) {
        submitBtn.textContent = `Hátralévők feloldása (${remaining})`;
    ...
    async function onSubmit() {
        ...
        if (!password) {
          if (errorEl) {
            const msg = useSame
              ...
              : `Adj meg jelszót a következőhöz: ${fileNames[i]}, vagy hagyd ki`;
      ...
      for (let i = 0; i < toProcess.length; i++) {
        ...
        if (progressEl) {
          progressEl.textContent = `Ellenőrzés (${i + 1}/${toProcess.length}): ${files[realIdx].name}`;
          progressEl.classList.remove('hidden');
        }
        ...
        if (progressEl) {
          progressEl.textContent = `Visszafejtés (${i + 1}/${toProcess.length}): ${files[realIdx].name}`;
        }
