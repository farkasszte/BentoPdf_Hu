...
(async function init() {
  try {
    ...
    toast('A WASM-motor betöltése nem sikerült: ' + e.message);
  ...
  if (demo) {
    try {
      ...
      toast('Bemutató betöltése nem sikerült: ' + e.message);
    }
  }
...
function restore(from, to) {
  ...
  try {
    ...
    from.push(snap);
    toast('A visszavonás nem sikerült: ' + err.message);
    return;
  }
...
async function spellStart() {
  ...
  $('spellStatus').textContent = 'Szótár betöltése…';
  const ok = await spellDictionary();
  if (!ok) {
    $('spellStatus').textContent = 'Nincs szótár.';
    return;
  }
  spellRescan();
  spellNext();
...
function wireBlockClipboard() {
  document.addEventListener('copy', (e) => {
    if (!state.editing && state.textSel?.text) {
      ...
      e.preventDefault();
      toast('A kijelölt szöveg átmásolva.');
      return;
    }
    ...
    e.preventDefault();
    toast('Blokk átmásolva – illeszd be, hogy stílusokkal másold.');
  });
...
function drawGuides() {
  ...
  const mk = (vertical, coord) => {
    ...
    g.title = 'Húzással mozgatható, dupla kattintással törölhető';
...
function wireUI() {
  ...
  $('altText')?.addEventListener('click', () => {
    ...
    if (mcid < 0) {
      toast('Ehhez a képhez nem tartozik szerkezeti címke.');
      return;
    }
    const current =
  ...
  $('extEdit')?.addEventListener('click', async () => {
    ...
    if (!img) {
      toast('A kép exportálása nem sikerült.');
      return;
    }
    const cv = document.createElement('canvas');
    cv.width = img.width;
  ...
  $('copyText')?.addEventListener('click', async () => {
    ...
    if (!t || !t.text) {
      toast('Ezen az oldalon nincs szöveg.');
      return;
    }
    try {
      ...
      toast('A vágólap-hozzáférés engedélyezése elutasítva.');
    }
  });
...
function duplicateSelection() {
  if (state.selection?.kind === 'object') {
    if (state.selection.type !== OBJ.IMAGE) {
      toast('A másolás jelenleg csak képeket és szövegdobozokat támogat.');
      return;
    }
...
async function loadSystemFonts(silent = false) {
  ...
  try {
    ...
    if (!silent) toast('A betűtípusok elérése elutasítva: ' + err.message);
  }
}
