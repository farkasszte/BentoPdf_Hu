...
function drawNormalized(
  ...
  if (!context) {
    throw new Error('Az összehasonlító vászon létrehozása nem sikerült.');
  }
...
export function renderVisualDiff(
  ...
  if (!context1 || !context2 || !outputContext) {
    throw new Error('A vizuális összehasonlító felület létrehozása nem sikerült.');
  }
  ...
  if (!overlayContext) {
    throw new Error('A vizuális összehasonlító réteg létrehozása nem sikerült.');
  }
