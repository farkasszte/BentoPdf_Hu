...
async function renderPage(num: number) {
  ...
  showLoader(`${num}. oldal betöltése...`);
  ...
  try {
    ...
    showAlert('Megjelenítési hiba', 'Az oldal megjelenítése nem sikerült.');
  } finally {
...
export async function setupCanvasEditor(toolId: string) {
  ...
  prevButton.className =
    'btn p-2 rounded-full bg-slate-700 hover:bg-slate-600 disabled:opacity-50';
  ...
  pageInfo.append(
    'Page ',
    currentPageDisplay,
    ` / ${editorState.pdf.numPages} oldal`
  );
      ...
      async () => {
        if (Object.keys(editorState.cropBoxes).length === 0) {
          showAlert(
            ...
            'Rajzolj egy téglalapot legalább egy oldalon a vágási terület kijelöléséhez.'
          );
          return;
        }
