  ...
  try {
    ...
    if (toolId === 'view-metadata') {
      ...
      try {
        ...
        const createSection = (title: string) => {
          ...
          ul.className =
            'space-y-3 text-sm bg-slate-950 p-4 rounded-lg border border-slate-700';
        ...
        const createListItem = (key: string, value: string) => {
          ...
          strong.className = 'w-40 flex-shrink-0 text-slate-400';
        ...
        if (info && Object.keys(info).length > 0) {
          ...
          infoSection.ul.innerHTML = `<li><span class="text-slate-500 italic">– Nem található Info-szótár adat –</span></li>`;
        }
        ...
        if (fieldObjects && Object.keys(fieldObjects).length > 0) {
          ...
          fieldsSection.ul.innerHTML = `<li><span class="text-slate-500 italic">– Nem található interaktív űrlapmező –</span></li>`;
        }
        ...
        const createXmpListItem = (key: string, value: string, indent = 0) => {
          ...
          const strong = document.createElement('strong');
          strong.className = 'w-56 flex-shrink-0 text-slate-400';
          strong.textContent = key;
        ...
        const createXmpHeaderItem = (key: string, indent = 0) => {
          ...
          const strong = document.createElement('strong');
          strong.className = 'w-full flex-shrink-0 text-slate-300 font-medium';
          strong.textContent = key;
          strong.style.paddingLeft = `${indent * 1.2}rem`;
          li.append(strong);
          return li;
        ...
        if (rawXmpString) {
          try {
            ...
            if (xmpSection.ul.children.length === 0) {
              xmpSection.ul.innerHTML = `<li><span class="text-slate-500 italic">– Nem található értelmezhető XMP-tulajdonság –</span></li>`;
            }
          } catch (xmlError) {
            console.error('Failed to parse XMP XML:', xmlError);
            xmpSection.ul.innerHTML = `<li><span class="text-red-500 italic">– Hiba az XMP XML feldolgozásakor, nyers formában jelenik meg –</span></li>`;
            const pre = document.createElement('pre');
            pre.className =
              'text-xs text-slate-300 whitespace-pre-wrap break-all';
            pre.textContent = rawXmpString;
            xmpSection.ul.appendChild(pre);
          }
        } else {
          xmpSection.ul.innerHTML = `<li><span class="text-slate-500 italic">– Nem található XMP-metaadat –</span></li>`;
        }
        ...
        showAlert(
          'Hiba',
          'A PDF teljes elemzése nem sikerült. Lehet, hogy sérült vagy szokatlan a felépítése.'
        );
      } finally {
        hideLoader();
      }
    ...
    if (toolId === 'edit-metadata') {
      ...
      addBtn.onclick = () => {
        ...
        keyInput.className =
          'custom-meta-key w-full sm:w-1/3 bg-slate-800 border border-slate-600 text-white rounded-lg p-2';

        const valueInput = document.createElement('input');
        valueInput.type = 'text';
        valueInput.placeholder = 'Value (e.g., Marketing)';
        valueInput.className =
          'custom-meta-value w-full sm:flex-grow bg-slate-800 border border-slate-600 text-white rounded-lg p-2';

        const removeBtn = document.createElement('button');
        removeBtn.type = 'button';
        removeBtn.className =
          'btn p-2 text-red-500 hover:bg-slate-700 rounded-full self-center sm:self-auto';
    ...
    showAlert(
      'Hiba',
      'A PDF betöltése nem sikerült. A fájl érvénytelen, sérült vagy jelszóval védett lehet.'
    );
    console.error(e);
  }
}
...
async function handleMultiFileUpload(toolId: string) {
  if (
    ...
    showLoader('PDF-dokumentumok betöltése...');
  ...
  if (toolId === 'alternate-merge') {
    ...
    state.files.forEach((file, index) => {
      ...
      const wrapper = document.createElement('div');
      wrapper.className =
        'w-full h-36 sm:h-40 md:h-44 bg-slate-950 rounded-md border-2 border-slate-600 flex items-center justify-center overflow-hidden';
